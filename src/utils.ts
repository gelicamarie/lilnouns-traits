import NounAssets from '../public/lilnouns-assets.json'
import { NounSeed, NounData, DecodedImage } from './types'

export const { images, bgcolors, palette } = NounAssets
export const { bodies, accessories, heads, glasses } = images
...
export const buildSVG = (parts: { data: string }[], paletteColors: string[], bgColor: string): string[] => {
  return parts
    .map((part) => {
      ...
      return svgRects.join('')
    })
    .map(
      (rects) =>
        `<svg width="320" height="320" viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" shape-rendering="crispEdges"><rect width="100%" height="100%" fill="transparent" />${rects}</svg>`,
    )
}
